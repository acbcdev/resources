...
export type FetchMethod = "fetch" | "playwright" | "playwright-headless" | "AI";
...
export class Logger {
  ...
  private formatMessage(message: string, options?: LogOptions): string {
    ...
    if (
      SCRIPTS_CONFIG.logging.showPrefix &&
   ...
   */
  private buildStatusString(status: {
    successful: number;
    warnings: number;
    failed: number;
  }): string {
    const parts: string[] = [];
    parts.push(
      `${this.colorize(String(status.successful), "green")}${this.getStatusIcon("success")}`,
    );
    parts.push(
      `${this.colorize(String(status.warnings), "yellow")}${this.getStatusIcon("warning")}`,
    );
    parts.push(
      `${this.colorize(String(status.failed), "red")}${this.getStatusIcon("error")}`,
    );
    return parts.join(" ");
  }
  ...
  tableWithColors(data: Record<string, number | string>): void {
    ...
    for (const [key, value] of entries) {
      ...
      const coloredValue =
        color !== "reset" ? this.colorize(valueStr, color) : valueStr;
  ...
  networkRequest(
    ...
    const methodLabel =
      method === "fetch" ? "Fetch" : `Playwright ${operationType}`;
