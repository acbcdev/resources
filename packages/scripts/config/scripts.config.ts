...
const DATA_DIR = resolve(PROJECT_ROOT, "./packages/scripts/data");
...
export const SCRIPTS_CONFIG = {
  ...
  ai: {
    ...
    temperature: 0.7,
    // Max content to send to AI (characters) - kept for backwards compatibility
...
export function validateConfig() {
  ...
  if (missing.length > 0) {
    ...
    console.warn(
      "📝 Some scripts may not work properly without these API keys.",
    );
  }
}
