...
async function main() {
  ...
  try {
    ...
    const allUrls = await fileIO.readJSONArray<string>(
      SCRIPTS_CONFIG.paths.input.newUrls,
    );
    ...
    logger.info(
      `${urlsToProcess.length} new URLs to process (${existingUrls.size} already done)`,
    );
    ...
    if (failedUrls.length > 0) {
      ...
      logger.info(
        `Failed URLs saved to: ${SCRIPTS_CONFIG.paths.output.failedOG}`,
      );
    }
