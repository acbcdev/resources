...
import type { Tool, ToolInput } from 'web/src/features/common/types/resource';
import type { Category as CategoryType } from 'web/src/features/common/types/category';
import type { TargetAudience as AudienceType } from 'web/src/features/common/types/audience';
...
async function getResourcesByEntity<T extends { id: number; name?: string }>(
	...
	const entity = (await db
		.select()
		.from(entityTable)
		.where(eq(entityTable.name, entityName))
		.get()) as T | undefined;
	if (!entity) return [];
...
export async function getResourcesByCollection(collectionName: string): Promise<Tool[]> {
	return getResourcesByEntity(
		...
		collectionName,
		ResourceCollection,
		ResourceCollection.collectionId,
	);
}
	...
	if (tool.targetAudience && tool.targetAudience.length > 0) {
		for (const audName of tool.targetAudience) {
			const aud = await db
				.select()
				.from(TargetAudience)
				.where(eq(TargetAudience.name, audName))
				.get();
			if (aud) {
