...
import { SEARCH } from 'web/src/features/common/consts/text';
	...
	return (
		<div className="search-input-container">
			<div className="search-wrapper">
				...
				/>
				<kbd className="keyboard-hint">{isMac ? '⌘' : 'Ctrl'} K</kbd>
			</div>

			{!isLoading && showDropdown && results.length > 0 && (
				<div ref={dropdownRef} id="search-results" className="search-dropdown" role="listbox">
					{results.map((result, index) => (
						...
						>
							...
							<div className="result-category">{result.category.join(', ')}</div>
						</a>
					))}
				</div>
			)}
			...
			{!isLoading && showDropdown && query.length > 2 && results.length === 0 && (
				<div className="search-dropdown">
					<div className="search-no-results">{SEARCH.noResults(query)}</div>
				</div>
			)}
